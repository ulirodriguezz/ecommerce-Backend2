...
public class JwtAuthFilter extends OncePerRequestFilter {
    ...
    protected void doFilterInternal(HttpServletRequest request, HttpServletResponse response, FilterChain filterChain)
        ...
        try {
            ...
            if (token != null && validateToken(token)) {
                ...
                List <GrantedAuthority> authorities = extractRolFromToken(token);
                if (username != null) {
    ...
    private String extractJwtFromRequest(HttpServletRequest request) {
        ...
        if (bearerToken != null && bearerToken.startsWith("Bearer ")) {
            ...
            return this.token; // Excluir "Bearer "
        }
    ...
    public String getUsername() {
        return this.extractUsernameFromToken(this.token);
    }
    private List<GrantedAuthority> extractRolFromToken(String token){
        try {
            Claims claims = Jwts.parserBuilder().setSigningKey(secretKey).build().parseClaimsJws(token).getBody();
